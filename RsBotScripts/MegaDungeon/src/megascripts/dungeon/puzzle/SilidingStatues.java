...
public class SilidingStatues extends Puzzle {
	
	public static int[] SILIDING1 = { 10958, 10954, 10962 }, SILIDING2 = {
			10959, 10955, 10963 }, SILIDING3 = { 10960, 10956, 10964 },
			SILIDING4 = { 10961, 10957, 10965 };
	public static int[][] SILIDING_STATUE = { SILIDING1, SILIDING2, SILIDING3,
			SILIDING4 };

	public static int[] STATIC1 = { 10946, 10942, 10950 }, STATIC2 = { 10947,
			10943, 10951 }, STATIC3 = { 10948, 10944, 10952 }, STATIC4 = {
			10949, 10945, 10953 };
	public static int[][] STATIC_STATUE = { STATIC1, STATIC2, STATIC3, STATIC4 };

	public static boolean Solved = false;

	public Tile Solve_Tile[] = null;

	@Override
	public String getName() {
		return "Siliding Statues";
	}
	...
	public boolean isValid() {
		...
		if(NPCs.getNearest(SILIDING_STATUE[0]) == null){
			return false;
		}
		return ulits.tileinroom(NPCs.getNearest(SILIDING_STATUE[0]).getLocation());
	}
	...
	public void solve() {
		if (MyActions.ThreNPC()) {
			...
			if (g == 100) {
				...
				Solve_Tile = findPath(
						MyNpc.getNearstNPCTo(STATIC_STATUE[g],
								SILIDING_STATUE[g]).getLocation(),
						getSolution(g));
				if (Solve_Tile.length > 0) {
					NextTile = Solve_Tile[1];
				} else {
					NextTile = null;
				}
				NPC SILIDE_STATUE = NPCs.getNearest(SILIDING_STATUE[g]);
				if (currRoom.MatchTile(NextTile)
						|| (MyNpc.getAt(NextTile) != null & Calculations
								.distanceTo(Solve_Tile[1]) < 2)) {
					if (MyNpc.getAt(NextTile) != null) {
						NextTile.interact("Push");
					} else {
						SILIDE_STATUE.interact("Pull");
					}
					ShadowDungeon.sleep(1000, 1500);
					ShadowDungeon.SleepTillStop();
				} else {
					if (currRoom.isOnScreen(NextTile.getCentralPoint())) {
						NextTile.interact("Walk here");
					} else {
						MyPlayer.WalkTo(NextTile);
					}
					ShadowDungeon.sleep(500, 900);
					ShadowDungeon.SleepTillStop();
				}
	...
	/*
	 * private static Tile getNearstTile(Tile st,Tile NextTile) { for(Tile r :
	 * Flood.getArea().getTileArray()){ if(r !=null){ int Dist =
	 * (int)Calculations.distance(NextTile,r); int Dist1 =
	 * (int)Calculations.distance(r,st); if(Dist > 1 && Dist1 < 2){ return r; }
	 * } } return null; }
	 */
	private static int getNextStatues() {
		for (int x = 0; x < SILIDING_STATUE.length; x++) {
			if (!currRoom.MatchTile(getSolution(x),
					(NPCs.getNearest(SILIDING_STATUE[x]).getLocation()))) {
	...
	private static int getCount() {
		...
		for (int x = 0; x < SILIDING_STATUE.length; x++) {
			if (!currRoom.MatchTile(getSolution(x),
					(NPCs.getNearest(SILIDING_STATUE[x]).getLocation()))) {
				Tile[] path = findPath(
						MyNpc.getNearstNPCTo(STATIC_STATUE[x],
								SILIDING_STATUE[x]).getLocation(),
						getSolution(x));
				if (!ContainNPC(path)) {
					count++;
				}
	...
	private static boolean ContainNPC(Tile[] path) {
		for(Tile e : path){
			if(MyNpc.getAt(e) != null){
				return true;
			}
		}
		return false;
	...
	public static Tile getSolution(int x) {
		return new Tile(NPCs.getNearest(STATIC_STATUE[x]).getLocation().getX()
				+ (ver(x) ? 0 : getOffset(x)), NPCs
				.getNearest(STATIC_STATUE[x]).getLocation().getY()
				+ (ver(x) ? getOffset(x) : 0), 0);
	}

	public static boolean ver(int g) {
		return Math
				.round(NPCs.getNearest(STATIC_STATUE[g]).getOrientation() / 90.0) % 2 == 1;
	}

	public static int getOffset(int g) {
		if (Math.round(NPCs.getNearest(STATIC_STATUE[g]).getOrientation() / 90.0) > 1)
			return -7;
		return 7;
	}
	...
	private static List<Vertex> successors(final Vertex t) {
		...
		if (f_x > 0 && f_y > 0
				...
				&& (MyNpc.getAt(new Tile(x - 1, y - 1, z)) == null)) {
