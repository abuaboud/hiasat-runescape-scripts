...
public class LocalPathCustom extends Path {
	private static Tile end;
	private static Tile base;
	private static int[][] flags;
	private static int offX, offY;
	public static int[] TILES = {49546,49547,49548}, HOLES = {49549,49550,49551}, PADS = {49555,49556,49557};
	public static int PAD = 0 , TILE =0;
	private static TilePath tilePath;

	public LocalPathCustom(final Tile end) {
		this.end = end;
	}
	...
	public static  Tile[] findPath(final Tile start, final Tile end) {
		PAD = SceneEntities.getNearest(PADS).getId();
		TILE = SceneEntities.getNearest(TILES).getId();
	...
	public static LinkedList<Vertex> Successors(final Vertex t){
        ...
        int x = t.x, y = t.y;
        try {
            if (SceneEntities.getAt(new Tile(x + 1, y,0), SceneEntities.TYPE_FLOOR_DECORATION).getId() == TILE ||
                SceneEntities.getAt(new Tile(x + 1, y,0), SceneEntities.TYPE_FLOOR_DECORATION).getId() == PAD)
                tiles.add(new Vertex(x + 1, y,0));
        } catch (Exception e) {
        }
        try {
            if (SceneEntities.getAt(new Tile(x + 1, y + 1,0), SceneEntities.TYPE_FLOOR_DECORATION).getId() == TILE ||
                SceneEntities.getAt(new Tile(x + 1, y + 1,0), SceneEntities.TYPE_FLOOR_DECORATION).getId() == PAD)
                tiles.add(new Vertex(x + 1, y + 1,0));
        } catch (Exception e) {
        }
        try {
            if (SceneEntities.getAt(new Tile(x, y + 1,0), SceneEntities.TYPE_FLOOR_DECORATION).getId() == TILE ||
                SceneEntities.getAt(new Tile(x, y + 1,0), SceneEntities.TYPE_FLOOR_DECORATION).getId() == PAD)
