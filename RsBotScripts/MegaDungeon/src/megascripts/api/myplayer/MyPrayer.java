...
public class MyPrayer {
	...
	public static final int SETTING_MODERN_SET_ACTIVE = 3272;
	public static final int SETTING_MODERN_ACTIVE_EFFECTS = 3274;
	public static final int SETTING_MODERN_QUICK_PRAYERS = 1769;
	...
	public static final int WIDGET_PRAYER_REMAINING = 6;
	...
	public enum Modern implements Effect {
		...
		PROTECT_FROM_MAGIC(11, 0x40, 37, "Protect from magic"),
		PROTECT_FROM_MISSILES(12, 0x80, 40, "Protect from missiles"),
		PROTECT_FROM_MELEE(13, 0x100, 43, "Protect from melee"),
	...
	public static int getPointPercentage() {
		return (int) (((getRemainingPoints() / ((float) Constants.PRAYER_LEVEL * 10))) * 100);
	}
	...
	public static int getRemainingPoints() {
		final WidgetChild prayer = Widgets.get(WIDGET_PRAYER_ORB, WIDGET_PRAYER_REMAINING);
		return prayer != null && prayer.validate() && prayer.getText().length() > 0 ? Integer.valueOf(prayer.getText()) : 100;
	}
	...
	private static boolean isQuickPrayerSet(Effect effect) {
		final WidgetChild quickPrayerSelection = Widgets.get(WIDGET_PRAYER, WIDGET_QUICK_PRAYER_SELECTION);
		if (quickPrayerSelection != null && quickPrayerSelection.validate()) {
			final WidgetChild currentSelection = quickPrayerSelection.getChild(effect.getComponentIndex());
			if (currentSelection != null && quickPrayerSelection.validate()) {
	...
	public static boolean setActivated(final Effect effect, final boolean activate) {
		if (activate != isActive(effect) && (!activate || effect.getRequiredLevel() <= Constants.PRAYER_LEVEL && getRemainingPoints() > 0)) {
			if(isActive(effect) != activate){
			if (Tabs.getCurrent() != Tabs.PRAYER) {
				Tabs.PRAYER.open();
				Task.sleep(700);
			}
			if (Widgets.get(WIDGET_PRAYER, 10).getChild(effect.getComponentIndex()).interact(activate ? "Activate" : "Deactivate")) {
				for (int i = 0; i < 30; i++) {
					if (isActive(effect)) {
						return true;
					}
	...
	public static boolean setQuickPrayers(Effect... effects) {
		if (!isQuickPrayersActive()) {
			final WidgetChild prayerOrb = Widgets.get(WIDGET_PRAYER_ORB, 1);
			if (prayerOrb != null && prayerOrb.validate() && prayerOrb.interact("Select quick prayers")) {
				for (int i = 0; i < 10 && (Widgets.get(WIDGET_PRAYER, WIDGET_QUICK_PRAYER_SELECTION) == null ||
						Widgets.get(WIDGET_PRAYER, WIDGET_QUICK_PRAYER_SELECTION).getChildren() == null ||
						Widgets.get(WIDGET_PRAYER, WIDGET_QUICK_PRAYER_SELECTION).getChildren().length == 0); i++) {
		...
		for (Effect effect : effects) {
			...
			if (quickPrayerSelection != null && quickPrayerSelection.validate()) {
				final WidgetChild currentSelection = quickPrayerSelection.getChild(effect.getComponentIndex());
				if (currentSelection != null && currentSelection.validate()) {
					currentSelection.interact("Select");
					Task.sleep(Random.nextInt(750, 1100));
				}
			}
		}
		if (isQuickPrayerSet(effects)) {
			final WidgetChild confirmSelection = Widgets.get(WIDGET_PRAYER, WIDGET_QUICK_PRAYER_CONFIRM);
			if (confirmSelection != null && confirmSelection.validate() && confirmSelection.interact("Confirm Selection")) {
				return true;
			}
		}
		return false;
	...
	/**
	 * @param active <tt>true</tt> to turn quick prayers on; <tt>false</tt> to turn
	 *               quick prayers off.
	...
	public static boolean toggleQuickPrayers(boolean active) {
		return active == isQuickPrayersActive() ||
				Widgets.get(WIDGET_PRAYER_ORB, 1).interact(active ? (isModernSetActive() ? "Turn quick prayers on" : "Turn quick curses on") :
